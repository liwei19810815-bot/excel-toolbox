...
| 可见性 | 这个人**能不能看到、能不能用**这个功能 |

所以可见性对**不在白名单的人同样生效**——只对白名单内生效的话，
把功能关掉之后大多数人照样能用，开关等于没有。

配置在网关的 `server/feature.json`，改完**不用重启**：

```json
{
  "visibility": 1,
  "visibilityOverrides": { "zhangsan": 2 }
}
```

| 值 | 含义 | **在哪儿兑现** |
|---|---|---|
| `0` | 不可见 | **安装时不注册加载项**；已装过的会在下次安装时被移除 |
| `1` | 可见可使用 | 默认 |
| `2` | 可见但置灰 | 照常装，运行时由任务窗格停用 |

### ⚠ 三态能做到什么程度，要说清楚

**`0` 只能在安装侧兑现。** Office.js **没有隐藏**内置选项卡上按钮的能力
（`Office.ribbon.requestUpdate` 只能改 `enabled`，改不了 `visible`）。
所以"真正看不见"＝安装时就不注册这个加载项。
对已经装上的机器，要等他下次跑安装程序才会被移除；在那之前按钮还在，
但点开会看到"已停用"。

**按钮置灰是尽力而为，不是强制点。** 没有共享运行时（shared runtime）时，
置灰的代码只有在用户**点开过任务窗格之后**才会跑——第一次点开之前
按钮看起来是正常的。
...
| 位置 | 内容 |
|---|---|
| `server/gateway.mjs` | 网关最小实现：静态托管 + `/api/ai-config`，零依赖 |
| `server/whitelist.txt` | 白名单，一行一个账号，**改完不用重启** |
...
- **managed 的配置不落 localStorage**。IT 下发的地址和 key 只活在内存里，
  ...
  配置，管控就是假的。
- **白名单读不出来时当作"谁都不在"**，不是"谁都在"。配置出问题的时候，
  少给权限永远比多给权限安全。
- **managed 下设置页的连接项全部锁住**，且不能用"导入配置"绕过——
  锁了输入框却留着导入口子等于没锁。
- **`enableRunScript` 一旦被 IT 下发就不许本机改回来**，否则"IT 能集中
  关掉"只是句空话。
