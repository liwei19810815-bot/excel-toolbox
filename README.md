...
**v1.0.0**：M1–M9 共 60 个命令，**每个命令都有回归测试**（227 个断言，命令覆盖 60/60），
...
不是"文本转数值"）。只命中一条时**直接执行那条命令**——
用户要的是把问题解决掉，多一次点击就多一次放弃的机会。

### 遥测

挂在 `modAction.RunAction` 的每个出口，**59 个命令零改动**——这是统一管线最大的红利。
