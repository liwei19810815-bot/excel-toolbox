...
| `src/help/help.md` | 什么时候用、怎么用、**示例**、有什么坑、搜索关键词 |

构建时把 `help.md` 解析后写进 `.xlam` 里一个 `xlSheetVeryHidden` 的工作表。
**不需要服务器，也不需要额外分发文件**——整个工具箱仍然只有一个 `.xlam`。

呈现方式是**贴在 Excel 右侧的帮助侧边栏**（`src/ui/frmHelpPane.vba`）：
搜索框 + 分组目录 + 正文 + 环境体检，可以一直开着边看边操作。

> **它不是 Office 原生的任务窗格。** 真正的 CustomTaskPane 只有 COM 加载项
> 能创建（通过 `ICTPFactory` 交给实现 `IDTExtensibility2` 的 COM 加载项），
> 纯 VBA 的 `.xlam` 拿不到。所以这是一个贴右缘的无模式窗体——能常驻、
> 能边看边操作，但**不会**像原生窗格那样把表格区域挤窄。UI 文案按实际写，
> 不自称"窗格"。

> **仓库里没有 `.frm`/`.frx`。** `.frm` 必须配一个同名 `.frx` 二进制资源，
> 控件布局就存在那个二进制里，git 看不出差别、评审也无从审起。
> 所以构建时新建空窗体、注入 `src/ui/*.vba` 的代码，控件全部由那段代码
> 在运行时 `Controls.Add` 生成——布局是可 diff 的。
> 代价是运行时控件的事件**接不到窗体代码模块上**，要靠 `clsPaneCtl`
> 用 `WithEvents` 包一层（漏了这一层的表现是"点了没反应"且不报错）。

目录分组按 `actionId` 前缀派生，不另存一份分组表。
`check-help.ps1` 断言**每个命令都落在某个分组里**——将来加了新前缀却忘了
加分组，那些命令会从目录里静默消失，功能还在、按钮还在，只是用户找不到。

另一条路是**弹出网页版**（侧边栏底部有入口，功能区「帮助」也走这条）：
左侧常驻目录、右侧正文，目录带筛选框，滚动时高亮当前条目。
要打印或全文检索时也是这条更合适。

每条命令的正文按固定顺序组织：
**什么时候用 → 处理逻辑 → 怎么用 → 示例 → 动画演示 → 注意**。

「处理逻辑」讲的是它**到底对数据做了什么**——"怎么用"只说点哪里，
而用户敢不敢点，取决于前者。

「动画演示」**不是每条都要有**，只在真有数据变化时才加——第一版
曾经"每条都必须有"，结果自检、关于这类没有数据变化的条目被硬塞了
假演示（旁白当演示、前后一模一样），比没有更糟。它是**纯数据驱动**的，
有两种格式：
...
变化只发生在一个值上时用这种，网页上渲成一个小播放器来回切换。
整行消失、行列对调、单元格合并、条件格式上色这类**多行多列的变化**，
一行文字演不出来，改用网格：
...
网页上渲两张表来回切，哪些行/格变了由 JS **按每行第一列的值配对**、
逐格比对得出，不用手工标高亮。`^` 表示和上面那格纵向合并（演示"合并
相同项"这类命令），`*文本` 表示这一格要高亮。

> **故意不用 GIF。** 二进制资源 git 看不出差别、评审无从审起，
> 改一个字都要重新录制。
>
> **看不见的字符要写成可见记号**（空格写 `␣`）：直接写真空格，
> 网页上根本看不出来，动画等于没演示——清除空格这类命令尤其如此。
> `check-help.ps1` 有断言守着两种格式，以及"help.md 里有几段动画、
> 网页上就得有几个播放器"——解析写错时动画会**安静地退化成普通段落**，
> 页面照常打开、不报任何错，数量对不上是唯一的信号。

> 为什么用工作表而不是生成一个 `.bas`：中文正文嵌进 VBA 字符串字面量要处理
> 单行长度上限、续行数上限和引号转义，踩雷就是编译错误。`.xlam` 本身就是工作簿。

功能区「帮助」组里的「我要做什么」搜索框、以及侧边栏顶部的搜索框，都按**症状**匹配
（用户搜的是"求和是0"，不是"文本转数值"），命中结果一律用帮助展示。

侧边栏目录最前面是 `guide.*` 的**使用配置**条目（宏被禁用、每次弹安全警告、
文件被网络锁定、看不到选项卡等）。它们不是命令、没有 `actionId`，
标题写在 `help.md` 的 `标题:` 行里，构建时进 `_Help` 表的第 4 列。

底部的「检查我的环境」只出一份只读报告，**绝不替用户改任何安全设置**——
信任中心、受信任位置属于安全设置，插件替用户改等于替他降低防护等级。
查不到的项（如受信任位置，读注册表要用的 `WScript.Shell` 会被静默拦截）
**如实写"无法自动检测"，不假装检测**。这两条都有断言守着。
...
> **这条后来又被踩了一次，代价是静默删数据。** `modText.CleanText` 过滤控制字符时
> 直接写了 `If AscW(ch) >= 32`，于是 U+8000 以上的字符一律判负、被当成控制字符删掉——
> 而那个区间正好是大量高频汉字（辰 说 财 货 购 路 车 运 通 部 采 里 金 银 销 长 问 间 题 风 高……）。
> 实测「北辰科技」被「清除空格」洗成「北科技」，**不报任何错**。
> 当时 283 条断言一条没红，因为文本用例全是 ASCII。
> 现在码位一律走 `modStr.CodePointOf`，**不要再自己写 `AscW`**；
> 中文用例也补进了 `run-tests.ps1`。
