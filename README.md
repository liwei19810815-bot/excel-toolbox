...
**v1.1.2**：M1–M9 共 60 个命令，**每个命令都有回归测试**（全量回归 385 个断言，命令覆盖 60/60），
...
「动画演示」在网页上会渲染成一个小播放器，在"处理前/处理后"两个状态之间
来回切换，变化的单元格高亮。它是**纯数据驱动**的：
...
> `check-help.ps1` 有断言守着这两点，以及"help.md 里有几段动画、
