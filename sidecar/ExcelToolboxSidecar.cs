...
namespace ExcelToolbox.Sidecar
{
    ...
    internal sealed class SidecarServer
    {
        ...
        //---------------------------------------------------------------------
        // 【调用宏的确认票——这是一致性/防重放层，不是身份边界】
        //
        // 【重要，别把这段代码的作用想得比它实际做到的更多】：
        // /confirm-macro 认的还是同一枚 X-Toolbox-Token，没有引入任何
        // "只有真人点了任务窗格确认弹窗才能拿到"的凭据。所以任何拿到
        // 有效令牌的调用方，都能自己先 POST /confirm-macro 换票，
        // 再 POST /run-macro——这条票挡不住"持有令牌就能执行宏"，
        // 因为签发票本身就只需要这同一枚令牌。
        //
        // （这是独立评审两轮验收才厘清的：第一轮做成单步调用时评审说
        // "确认可被绕过"；这一版把它改成两步之后，评审指出两步换票
        // 认的是同一把钥匙，绕过路径依然成立，只是从一次请求变成两次。
        // 结论：**纯软件层做不出"证明用户点过确认"这件事**——区分
        // "任务窗格代用户点的"和"脚本自己发的"，本质上需要浏览器脚本
        // 无法伪造的东西（比如 sidecar 进程自己弹的原生对话框）；
        // 光靠 HTTP 层的令牌/票据都是同一个可复制的秘密，区分不出来。
        // 团队与用户讨论后的结论是：不引入原生弹窗，转而**明确把
        // "持有 sidecar 令牌"定义为"拥有执行宏的权限"**——不再对外
        // 宣称任务窗格的确认弹窗是 sidecar 强制的安全边界，那只是
        // UI 层面"减少误触发"的体验保护。）
        //
        // 这套票据机制仍然有它真实的价值，只是价值不在"防绕过"：
        //   · 单次性消费，挡得住"抓包重放同一条 /run-macro 请求"；
        //   · 绑定宏名+参数，换个参数原来的票就失效，减少"手滑传错参"
        //     被悄悄放行的窗口；
        //   · 120 秒短时效，减少令牌一旦泄露后可被长期利用的时间窗。
        // 但它证明不了"这次调用经过了用户确认"——不要在文档或对外说明
        // 里这样写。
        //---------------------------------------------------------------------
        ...
        private void Handle(TcpClient client)
        {
            ...
            using (var stream = client.GetStream())
            {
                ...
                // 换一张一次性票，绑定"这次要调的宏名 + 参数"。
                // 【认证条件和其他接口一样，就是这枚 X-Toolbox-Token】——
                // 不代表"调用方是走了任务窗格确认流程的"，任何持有令牌的
                // 调用方都能自己换。这一步本身不执行任何宏。
                ...
                //      confirmToken，否则拒绝执行。【这不是身份边界】——
                //      /confirm-macro 认的还是同一枚令牌，持有令牌者可以自己
                //      换票，这道门槛防的是重放和"参数被悄悄换掉"，不是
                //      "证明用户点过确认"。详见上面 _confirmTickets 那段注释。
