...
foreach ($sec in $sections) {
    ...
    # 【不是每条都要有动画】。第一版曾经"每条都必须有"，结果一批
    # 没有真实数据变化的条目（自检、关于、配置类说明）被硬塞了假演示——
    # 旁白当演示、前后一样、多格挤成一行。假演示比没有更糟，用户会
    # 以为页面坏了。现在只校验"有的话必须是真格式"，有没有交给写的人判断。
    #
    # 动画有两种格式：
    #   逐格演示：演示: 前 → 后（一行文字）
    #   网格演示：演示前: / 演示后: 后面跟 | 单元格 | 表格
    # 【写错不会报错，只会安静地退化成普通段落】——页面照常打开，
    # 只是动画没了，没有任何征兆，所以两种格式都要校验。
    if ($sec -match '(?m)^###\s+动画演示\s*$') {
        $demoLines = @([regex]::Matches($sec, '(?m)^演示[:：]\s*(.+)$') | ForEach-Object { $_.Groups[1].Value })
        $hasGrid = ($sec -match '(?m)^演示前[:：]\s*$') -and ($sec -match '(?m)^演示后[:：]\s*$')

        if ($demoLines.Count -eq 0 -and -not $hasGrid) {
            $errors += "动画演示一条都没有：$id（### 动画演示 下面要有「演示: 前 → 后」，或「演示前:/演示后:」加网格）"
        }
        ...
        if ($hasGrid) {
            # 网格必须有实际的行才叫演示，光有「演示前:」「演示后:」两个空标题没意义
            if ($sec -notmatch '(?m)^演示前[:：]\s*\r?\n\|') {
                $errors += "网格演示「演示前」下面没有表格行：$id（要跟着 | 单元格 | 这样的行）"
            }
...
try {
    ...
    if ($drive -notlike 'OK|*') {
        ...
        $checks = @(
            ...
            # HTML 按钮的失败原先被静默丢掉：生成失败时用户点了没有任何反应。
            # 现在成功失败都会在正文区留下话，这两条守住"一定有反馈"和"这次是成功的"。
            @{ Pat = 'htmlFeedback=True'; Msg = "点「在浏览器打开完整帮助」之后正文区没有任何反馈（失败被静默丢掉了）" }
            @{ Pat = 'htmlOk=True';       Msg = "完整帮助 HTML 生成失败（帮助内容没进加载宏？）" }
        )
        foreach ($c in $checks) {
